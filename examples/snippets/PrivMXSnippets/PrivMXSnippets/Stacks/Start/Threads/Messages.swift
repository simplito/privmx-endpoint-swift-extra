...
extension PrivMXSnippetClass{
	func sendMessageStart(){
		...
		let msgId: String = (try? endpointSession?.threadApi?.sendMessage(
			in: CONTEXT_ID,
	...
	func listMessagesStart(){
		...
		let pagedList = try? endpointSession?.threadApi?.listMessages(
			from: threadID,
			basedOn: query)
	...
	func updateMessageStart(){
		try? endpointSession?.threadApi?.updateMessage(
			"YOUR_MESSAGE_ID",
	...
	func deleteMessageStart(){
		try? endpointSession?.threadApi?.deleteMessage("YOUR_MESSAGE_ID")
	}
}

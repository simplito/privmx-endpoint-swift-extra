...
/// A helper extension for `StreamUpdatedEvent` to conform to the `PMXEvent` protocol.
/// This extension is designed to assist with event channels type conversions,
/// as channels are identified by strings in the Low-Level Endpoint.
extension privmx.endpoint.stream.StreamUpdatedEvent: PMXStreamEvent, @unchecked  Sendable {

	/// Returns the event type as a string.
	///
	/// This method returns the constant string `"StreamUpdated"`, identifying the type
	/// of this event as `StreamUpdated`.
	/// - Returns: A `String` representing the event type, in this case, `"StreamUpdated"`.
	...
	public func handleWith(
		...
		Task{
			await cb(StreamRoomUpdatedSubscriptionPayload.streamUpdated(data: data))
		}
	}
